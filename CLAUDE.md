...
- **Never** write `Match.is_data_verified` directly — always go through `set_data_verified()` in `matches/services/data_verification_service.py` (writes the `MATCH_DATA_VERIFIED_SET` audit trail; `reason` is mandatory). That flag decides whether a result is publicly visible (`result_visibility.py`, BLUEPRINT §14).
- **Never** write `MatchReport.publication_level` directly — it is written **only** by `publish_report()` (levels `FULL` | `SCORE_ONLY`, Opzione A). A `SCORE_ONLY` publish projects score/partials, creates **no** `MatchEvent`, and does **not** weaken Policy A strict: the zero-events abort stays identical on the `FULL` level.

...
- **`RED_CARD` è già un tipo canonico pubblicabile.** È fra i `DEFAULT_EVENT_TYPES` ([matches/event_types.py:8,15](../matches/event_types.py), label "Cartellino Rosso", `is_score=False`) ed è **proiettato in pagina pubblica**: `MatchDetailView` lo filtra e lo mostra (`red_cards`, [matches/views.py:46](../matches/views.py)) quando `events_published`. A differenza di `EXCLUSION_DEF`, `display_label` **trova** l'etichetta.
...
**Conseguenza sul disegno (NON riprogettata in questo giro — da ratificare):** la chiusura **non** è più "rendere canonico un tipo parallelo `EXCLUSION_DEF`" (che duplicherebbe `RED_CARD`, già canonico e già mostrato, sullo stesso evento), ma **"arricchire il `RED_CARD` esistente con i metadati sanzione"**: (a) al converter, **mappare** `EXCLUSION_DEF` → `RED_CARD` (o riconoscere che l'estrazione V3 già produce `RED_CARD`); (b) portare `regulation_article` + `sanction_sigla` + la classificazione da `classify_definitive_exclusion` fino al `MatchEvent` `RED_CARD` (nuovi campi o `notes` strutturate), invece di scartarli; (c) test di publish su un `RED_CARD`/EDCS riconciliato e uno non riconciliato. Non serve più un nuovo tipo nei `DEFAULT_EVENT_TYPES` né un nuovo `SportEventConfig` per un codice grezzo. La condizione di chiusura (a)/(b) sopra va quindi **rivista** in questo senso alla prossima presa in carico; il resto del debito (perdita silenziosa dei metadati al converter) è invariato.

### §10.36 Badge "AI Engine v2" nella dashboard è branding del Discovery, decorrelato dalla versione del prompt OCR — AMBIGUITÀ NOTA 2026-07-23 (severità cosmetica)

> **Severità:** cosmetica · **Aperta dal:** 2026-07-23 · **Condizione di chiusura:** decidere se il badge deve riflettere una versione reale (e allora legarlo a una sorgente, non a una stringa hardcoded) o restare puro nome di prodotto (e allora togliere il "v2" che si legge come numero di versione).

Nel riquadro **Caricamento Rapido** della dashboard ([templates/accounts/dashboard.html:164](../templates/accounts/dashboard.html)) c'è un badge hardcoded `AI Engine v2`. Verificato: sta dentro la sezione **"AI DISCOVERY QUICK UPLOAD"** (righe 156-172), il cui testo parla di *discovery* (identificare squadre/data/campionato dal contenuto), **non** di qualità/versione dell'estrazione OCR. La stringa è **statica**, non legata a `settings.OCR_PROMPT_VERSION` né ad alcuna altra sorgente di versione. **Non è quindi la versione del prompt OCR** (che è v3 in produzione dal 2026-07-23, §8.21, non v2): è branding della generazione del motore di discovery. **Lasciato invariato** (scelta corretta: non è stale rispetto a ciò che etichetta). L'ambiguità registrata: un lettore può leggere "v2" come numero di versione dell'AI/OCR e confonderlo con le versioni di prompt (v2/v3/V3.x). È decorativo e a blast radius nullo — nessuna logica dipende dal badge.

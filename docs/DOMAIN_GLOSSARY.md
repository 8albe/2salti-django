...
| Stagione | `League.season` (CharField) → `Season` (entità, deciso) | core | core/models.py | 🟡 | Oggi è un CharField su `League`, formato canonico `2025/2026` (slash). **Redesign Sprint D — Fase 1 implementata su dev (2026-06-09, syllabus Macro 16):** `Season` entità di prima classe (migration 0011, `is_current` per sport, al massimo una corrente per sport); FK transitoria `League.season_fk` (nullable, PROTECT) affiancata alla stringa `League.season`, che resta fino alla Fase 2 (rename lì). `Membership.season` e prestito ancora da fare (Fasi 2-4). Distinta da `SeasonArchive` (archivio storico stats): da linkare, non fondere. |
...
| Membership (appartenenza squadra) | `Membership` | management | ✅ | Lega User a Society+Team con role: PRESIDENT, HEAD_COACH, ASSISTANT_COACH, PLAYER. **Redesign Sprint D (deciso, non implementato — syllabus Macro 16):** campo `season` (FK a `Season`) e nuova unique key `(user, society, team, role, season)` in luogo di `start_date`/`end_date`. |
| Prestito (loan) | — (concetto deciso, nessun modello) | management | 📋 | **Solo deciso in BLUEPRINT v3.5 (Sprint D) — NON ancora nel codice: nessun campo, modello o migration esiste oggi.** Pianificato in syllabus Macro 16 §16.5: unica eccezione a "una società per stagione", solo squadre dei grandi (A1–D). Membership marcata prestito con FK alla società di tesseramento e stato attivo/concluso come **etichetta** (non macchina a stati). Constraint DB rigido: vietata 2ª società nella stessa stagione se non marcata prestito. |
| Tipo lega (grandi / giovanili) | — (concetto deciso, nessun campo) | core | 📋 | **Solo deciso in BLUEPRINT v3.5 (Sprint D) — NON ancora nel codice: nessun campo, modello o migration esiste oggi.** Pianificato in syllabus Macro 16 §16.4: la lega diventa la fonte di verità (si eliminerà `Team.category`). Lista chiusa: A1, A2, B, C, D = "dei grandi"; U10–U20 = giovanili. Giovanili con etichette tradizionali come display mappate 1:1 sull'Under canonico (U12=Esordienti, U14=Ragazzi, U16=Allievi, U18=Juniores; U10/U20 senza etichetta). |
...
| "Seasons / Stagioni" (entità §10) | Oggi la stagione è un CharField formato `2025/2026` (slash) su `League`; il redesign Sprint D (deciso, non implementato — syllabus Macro 16) la promuove a entità `Season`. `SeasonArchive` (seasons app) è cosa diversa: solo archivio storico delle stats | Stagione corrente = `Season.is_current` per sport (post-redesign); storico = `SeasonArchive`. I due concetti restano **distinti**: da linkare, non fondere |

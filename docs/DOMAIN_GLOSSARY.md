...
| Stagione | `League.season` (CharField) → `Season` (entità, deciso) | core | core/models.py | 🟡 | Oggi è un CharField su `League`, formato canonico `2025/2026` (slash). **Redesign Sprint D deciso, non implementato (syllabus Macro 16):** `Season` diventa entità di prima classe (FK da `League` e `Membership`, `is_current` per sport, al massimo una corrente per sport). Distinta da `SeasonArchive` (archivio storico stats): da linkare, non fondere. |

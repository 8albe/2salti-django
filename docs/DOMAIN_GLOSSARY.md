...
| Match_Jury_Links | `MatchJuryLink` | matches | matches/models.py | ✅ (dev) | **As-built 2026-07-19 (Macro 14, fondamenta backend):** link monouso per-partita, no-account, `ACTIVE→CONSUMED/REVOKED/EXPIRED`; un solo `ACTIVE` per match (partial unique index); token `secrets`; backstop 7gg valutato a lettura. Emissione/revoca `POST /api/matches/{id}/jury-link[/revoke/]` (gate staff digitale/admin — RBAC non society-scoped), accesso via token su start/update/close, landing `GET /r/{token}`, firma nome+cognome al close (`MatchReport.referee_signature`). Migration `0018`. **UI/QR/offline-first fuori scope.** Sostituisce il token federale 30-min (decaduto, FUTURE_IDEAS.md §1) — v. syllabus/14 §14.6-bis, STATE_MACHINES.md §12 |

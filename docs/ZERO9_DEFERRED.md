...
- [x] **Gate ramo CREATE di `create_society`** — fatto su dev (`bebf271`, 2026-07-06). Guard staff-only: se `existing is None` (nessuna `managed_society`) e l'utente non è staff, redirect a `choose_society`; lo staff mantiene CREATE come strumento operativo. Nessun redirect-loop (verificato: `choose_society` renderizza per un presidente senza società, non rimbalza). Rischio noto invariato: `choose_society` offre solo società già esistenti (`societies_for_personification()` filtra `teams__isnull=False`) — non c'è ancora un percorso self-service per una società genuinamente nuova; resta una decisione di prodotto separata.
- [ ] **Footgun `ensure_superuser.py`**: `core/management/commands/ensure_superuser.py` crea/aggiorna il superuser con password `'admin'` hardcoded nel sorgente — da parametrizzare via env in un giro dedicato (segnalato 2026-07-05, non fixato).

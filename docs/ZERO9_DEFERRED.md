...
- [x] **Pulizia account bot sul signup prod** — **ESEGUITA su prod il 2026-07-05** (Task 4). Criterio "UNVERIFIED + email vuota" confermato BANDITO (seleziona i 58 seed pilota, non bot, vedi sotto); criterio nuovo usato: 0 legami reali (Membership/AthleteProfile/CoachProfile/PresidentProfile) + firma spam. Recon read-only su backup prod, poi delete via management command `core/management/commands/cleanup_bot_users.py` (dry-run default, `--apply`, `--flush-sessions`, guard hard su staff/superuser e legami reali) eseguita da Alberto: **27 id cancellati** (18 certi + 8 husk + 1 test manuale id 89) = 54 record (27 `User` + 27 profili husk), utenti prod **86 → 59**, i 58 seed pilota e `albe_admin` intatti. Procedura in [OPS_RUNBOOK.md](OPS_RUNBOOK.md) §10.5. **Residuo aperto**: bot prod id ≥ 90 eventualmente nati dopo il backup delle 19:12 non sono stati caratterizzati (fuori da questa delete); `--flush-sessions` usa `clearsessions` globale, non per-utente. Honeypot anti-bot sul signup (`4aa48e7`, 2026-07-06) resta a prevenire nuovi bot self-service; rate-limit IP resta aperto.

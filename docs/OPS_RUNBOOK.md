...
**L'ordine restart↔migrate è un CRITERIO, non una sequenza fissa.** La regola "restart prima del migrate" nasce qui, sul caso della `0017` distruttiva, ma non è universale: quale finestra vada protetta dipende dalla **natura della migration**. Il criterio, verificato su due casi reali:
- **Migration distruttiva (`DROP`)** — il pericolo è **codice vecchio contro schema nuovo** (un processo in RAM tocca una tabella/colonna già cancellata): **restart prima**, così nessun processo attivo referenzia lo schema morto quando il `DROP` cade. È il caso `0017`.
- **Migration additiva con colonna `NOT NULL` senza `DEFAULT` a livello DB** — il pericolo si **inverte**: è **codice nuovo contro schema vecchio**. Il codice nuovo prova a `INSERT` con la colonna che ancora non esiste → `OperationalError`. In questo caso **migrate prima**: il codice vecchio invece legge senza problemi lo schema nuovo (seleziona colonne esplicite, non `SELECT *`) e fallisce solo sull'`INSERT` — quindi la finestra scomoda esiste solo nel verso migrate-dopo-restart. È il caso `0020` (deploy 2026-07-23, §2.12), eseguito **migrate → restart** e corretto.
- **In entrambi i casi**, prima del `pull` va fatto lo **stop preventivo del worker OCR** (`2salti-ocrworker`): il worker si auto-riavvia a coda vuota sul cambio di `HEAD`, quindi potrebbe ripartire sul codice nuovo mentre lo schema è ancora quello vecchio (o viceversa) senza che nessuno lo controlli. Fermarlo prima del pull chiude quella finestra; lo si riavvia esplicitamente a valle, nell'ordine deciso dal criterio.
...
### 2.11 Deploy 2026-07-22 — `2ad3436` → `9efc3d79` (filone OCR: check per-periodo, guardrail dato-verificato, criterio classifica blindato) + riparazione prereq2 su prod

Deploy reale dev→prod con lo stesso pattern di §2.5-§2.9 (merge `--no-ff` `dev`→`master` da home + push; pull su prod). Prod portato da `2ad3436` a **`9efc3d79`** (merge di `dev` a `f350e68`). **Delta senza alcuna migration**: verificato con `git diff --name-status master..dev -- '*/migrations/*'` (vuoto), `showmigrations matches` e query diretta su `sqlite_master`. Restart pieno `2salti` + `2salti-ocrworker` (worker su `revision=9efc3d79`, riavvio pulito nel journal), `curl -I https://2salti.com/` → **200**.

**Nota operativa — falso allarme `0017`, seconda occorrenza.** La session note di partenza dava di nuovo la migration distruttiva `matches/0017` (`DROP TABLE matches_ocrrawresponse`) come pendente su prod con rituale distruttivo da eseguire. È **stale**: la `0017` è applicata su prod dal **2026-07-19 08:52** (§2.6), tabella già droppata. È la **seconda volta** che questa stessa premessa stale fa scattare lo stesso allarme — la prima fu il 2026-07-21 (§2.9). La regola non cambia (la verifica pre-deploy a git + DB va fatta comunque, sempre), ma va messo a terra il fatto: **la memoria di sessione sulla `0017` è stale per definizione da 19/07 08:52** — qualunque nota che la dia pendente descrive uno stato anteriore a quell'istante e non va creduta senza il riscontro a `showmigrations` + `sqlite_master`. Stesso schema del gotcha §2.4: la narrativa di sessione non è la topologia.

**Riparazione prereq2 su prod (a seguire, stesso giorno).** Riparazione `normalized_data` dei 5 referti gold (7/8/10/11/16) su prod ai valori del gold standard — dettaglio in syllabus §8.5 (aggiornamento 2026-07-22). Checklist a 6 blocchi (`scratch/prereq2_repair_prod_20260722.sh`), gated su backup + dry-run su copia del backup con SHA del DB prod verificato invariato prima/dopo. Asserzione finale verde (blocker **7/7/7/4/5** contro attesi fissati prima, §6.5); verifica indipendente via `sqlite3` diretto (scores gold, `raw_extracted_data` invariato per hash, status `NEEDS_REVIEW`, 5 righe audit `normalized_data_repair_gold`); non-regressione Match 1/2/3/4 = gold e LeagueStanding a zero (nessun `PUBLISHED`). **Backup `/var/tmp/db.sqlite3.prereq2-prod-20260722`** (sha256 `62cf73c518439158a004a6db2ee546f3be63bff713473c5632d0265e5af34f75`) **da conservare fino a chiusura esplicita** di Alberto.

### 2.12 Deploy 2026-07-23 — `9efc3d79` → `539d126e` (Opzione A: `SCORE_ONLY` + migration additiva `0020`, prompt di produzione V2 → V3)

Deploy reale dev→prod con lo stesso pattern di §2.4–§2.11 (merge `dev`→`master` da home + push; pull su prod; `migrate` manuale gated dopo backup DB). Prod portato da **`9efc3d79`** a **`539d126e`** — delta `master..dev` di **34 commit**. Migration applicata **`0020`** (additiva: `MatchReport.publication_level`, `SCORE_ONLY`/`FULL`, Opzione A). **Il prompt OCR di produzione passa da V2 a V3** (default codificato dal commit `1a6af02`), primo giro in cui il default di produzione non è più V2. Worker OCR su revisione `539d126e21484c9421d4add662252c32ee4a3cd2` dalle **17:16:59**. **Backup `/var/tmp/db.sqlite3.opzionea-v3-20260723`** — **da conservare fino a chiusura esplicita** di Alberto (non ancora chiuso).

**Ordine eseguito: `migrate` → `restart`** (invertito rispetto al rituale distruttivo `0017`). La `0020` è additiva con colonna `NOT NULL` senza `DEFAULT` a livello DB: la finestra scomoda è **codice nuovo su schema vecchio** (`OperationalError` sull'`INSERT`), non il contrario — quindi migrate prima. Il criterio generale (distruttiva → restart prima; additiva-`NOT NULL`-senza-default → migrate prima; in entrambi i casi stop del worker prima del pull) è in §2.6. Smoke end-to-end verde su prod.

**Learning operativo — il dry-run va PRIMA del pull su prod.** Il dry-run della migration su copia scratch va eseguito usando **`git archive master`** come sorgente del codice nuovo, **prima** del `pull`: così si conosce l'esito del `migrate` senza mai portare il codice nuovo su disco accanto allo schema vecchio, e la finestra "codice-nuovo/schema-vecchio" **non si apre mai** sul box di produzione. Pullare prima del dry-run aprirebbe quella finestra per tutta la durata della verifica.

**Learning operativo — `chmod 600` sui backup del DB prod subito dopo la creazione.** I backup contengono dati personali di atleti **potenzialmente minorenni** e la VPS è **condivisa**: il file di backup va portato a `chmod 600` immediatamente dopo la creazione, non "poi". Vale per ogni `/var/tmp/db.sqlite3.*` di produzione.
...
I debiti aperti vivono in [DEBITI.md](DEBITI.md); l'archivio dei chiusi — incluse le voci della ex Appendice A — in [DEBITI_CHIUSI.md](DEBITI_CHIUSI.md). La numerazione §10.x prosegue in quei file: i nuovi debiti si aprono in DEBITI.md con ID progressivo (§10.32, §10.33, …) e alla chiusura la voce si sposta in DEBITI_CHIUSI.md. Questo runbook non ospita più voci di debito; i riferimenti §10.x nel resto del file e negli altri documenti puntano a quei due registri.
...
La voce vive in [DEBITI_CHIUSI.md](DEBITI_CHIUSI.md) (ID §12.9 conservato, fuori serie §10.x, per continuità di citazione — riorganizzazione registro debiti 2026-07-22, stessa regola della sezione 10): saldata da A2 il 2026-06-30; il "residuo aperto" `Sport.hex_color` del testo storico era già no-op alla stessa data (verifica in session note 30/06, ri-verificato il 2026-07-22). Questo runbook non ospita più voci di debito.

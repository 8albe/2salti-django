...
> (ex OPS_RUNBOOK §12.9, saldato da A2 il 2026-06-30 e archiviato in
> [DEBITI_CHIUSI.md](../DEBITI_CHIUSI.md) §12.9). Tutte le sotto-macro 17.1–17.8.1 chiuse → Macro 17 **✅**.
...
>
> ⚠️ **Residui aperti Fase 2** (la Macro resta 🔄): (a) **per-sport color DB-driven** (`Sport.hex_color`): codice pronto su `dev` (commit `e5eab0d`) — model default `#00ffff`→`#2563eb` (schema migration `0020`), data migration `0021` difensiva (solo `slug='pallanuoto'`, idempotente, reversibile), seed allineato. **CHIUSO (2026-06-30):** migration `0020`/`0021` applicata su `dev` e **propagata a prod** (deploy `24bfc62`) — `0021` data-migration ha toccato 1 row, pallanuoto su prod ora `#2563eb`. Il residuo "gated da backup+ratifica" è **chiuso**; (b) **literali ciano orfani** (`rgba(6,182,212,…)` glow + hex `#0891b2`/`#0e7490`/fallback `#06b6d4`) — CHIUSO su `dev` (commit `ac9b970`): 15 template portati a blue-600, grep literali ciano = 0; (c) **verifica e2e Antigravity** (dark+light, tutte le superfici, + ri-verifica sport-context post-migration) ancora da eseguire; (d) **debito semantico A1** (classi `cyan-*` che rendono blue) → vedi [DEBITI_CHIUSI.md](../DEBITI_CHIUSI.md) §12.9 (ex OPS_RUNBOOK §12.9, saldato da A2 il 2026-06-30).
...
- [x] Cambio DB `Sport.hex_color` pallanuoto ciano→blue — _(model+migration `0020`/`0021`+seed, `e5eab0d`; migration applicata su `dev` e **propagata a prod** 2026-06-30, deploy `24bfc62`; pallanuoto prod = `#2563eb`)_

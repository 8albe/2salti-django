...
### 8.6 Finding di discovery: l'OCR sbaglia il nome squadra

> **Riscritto il 2026-07-20. La versione precedente di questo paragrafo era sbagliata** e diceva l'opposto: che il referto cartaceo riportava `BELLATOR FROSINONE` mentre a DB la squadra è `Bellator Frusino`, cioè una divergenza di grafia foglio↔DB. Quella lettura del cartaceo era un **errore di collazione umana** del 2026-07-19, esposto dal bench e corretto su riverifica dell'originale il 2026-07-20 (`corrections` nel caso gold).

Sul referto cartaceo c'è scritto **`BELLATOR FRUSINO`**, che coincide con il nome a DB (`Bellator Frusino`, Team pk=5). Nessuna divergenza foglio↔DB. Entrambi i provider hanno però estratto **`BELLATOR FROSINONE`** (verificato in sola lettura su `normalized_data` di dev e prod il 2026-07-20: report 16 `gemini-2.5-pro`, report 10 `gpt-4o`), entrambi con `confidence_fields.home_team = 1.0`. Il referto 16 è finito orfano (`match=None`, `NEEDS_REVIEW`, "Impossibile risolvere una o entrambe le squadre") **pur esistendo la squadra a DB con il nome giusto**.

La causa reale è quindi un **errore di lettura OCR sul nome squadra**, non un fuzzy matching troppo stretto: la discovery ha ricevuto un input già corrotto a monte e non poteva agganciare nulla. `FRUSINO` è la forma latina di Frosinone e la parola `FROSINONE` compare altrove sullo stesso foglio (campo città): entrambi i modelli hanno normalizzato un nome proprio raro verso la forma comune più probabile — errore di prior linguistico, che il preprocessing non attenua e che la confidence auto-dichiarata non segnala. Aggravante di contesto: quel cartaceo è compilato molto male, al limite della leggibilità anche per un umano (è la condizione in cui è nato l'errore di collazione).

Resta un problema di **Macro 8, non di Macro 22**: l'asincrono si è limitato a renderlo visibile al primo upload reale. E resta sistematico, non occasionale — l'errore di prior si ripeterà su ogni referto di quella squadra.

**La direzione "tabella di alias" è però sospesa.** Un alias mappa grafie legittime alternative, non valori allucinati: su questo caso non avrebbe risolto nulla. I due casi che ancora la sostengono — `Olympic Roma P.N.` vs `Olimpic Roma P.N.` (Team pk=7) e `Nautilus Nuoto Roma` vs `Nautilus N. Roma` (Team pk=3) — vengono dalla **stessa sessione di collazione** che ha prodotto l'errore Bellator e sono marcati `pending_reverification` nei rispettivi casi gold, in attesa che Alberto torni sul cartaceo. Se anche quelli fossero errori di collazione, la fase alias perde tutti i suoi casi fondativi e il problema da attaccare è l'accuratezza OCR sui nomi propri.

...
**Verifica H_IMG — falsificazione dell'astensione come artefatto d'immagine (2026-07-23, dopo l'esito).** Prima di lasciare questa conclusione si è testata l'ipotesi alternativa che l'astensione di gpt-5 dipendesse dalla **fedeltà dell'immagine inviata**, non dal modello (il provider OpenAI era stato scritto da zero e collaudato solo con mock: i mock validano l'interfaccia, non che la chiamata reale sia ben formata). Ispezione della catena: **entrambi i bracci inviano lo stesso identico file** — `ImagePreprocessor.process()` è **unico**, non tarato su Gemini ([vision_providers.py:1003](../../matches/services/vision_providers.py#L1003) e :834) — il `detail` OpenAI è **già `high`**, il massimo dell'API ([vision_providers.py:1031](../../matches/services/vision_providers.py#L1031)), e sotto `detail:high` sia l'originale (2048×1430) sia il processato (2000×1396) collassano a **6 tile / ~1105 token immagine**: il downscale a 2000px del bench è **irrilevante** per OpenAI. Una chiamata di falsificazione — referto 8, gpt-5, `v3_4` invariato (`@sha256:4e9751eded9b`), `reasoning=high`, immagine alla **massima fedeltà** (`--no-preprocess`: originale grezzo, nessun downscale, nessuna ricompressione, `detail:high`) — ha comunque prodotto **zero eventi e zero gol** (`finish_reason` non `length`, `tok_thk=8064` consumati); ha estratto i punteggi (11/13 campi corretti) e, col grezzo, perfino il roster ospiti (garbled) che il processato non dava — cioè la max-fedeltà semmai *aiuta marginalmente*, ma **non recupera i gol**. **H_IMG FALSIFICATA**: l'astensione sugli eventi non è un artefatto dell'immagine ma la regola null-preference di `v3_4` applicata alla griglia cronometrica (H2), come già concluso sopra — la verifica **rafforza** l'esito, non lo indebolisce. Costo: **$0,094** (3.799 tok in / 8.889 out). Effetto collaterale: il collaudo mock-only del provider è sanato da una chiamata reale end-to-end che conferma l'invio dell'immagine a fedeltà massima.
...
### 8.24 Calottina come identità dell'autore + ritaglio reale della zona — stadio 1 CHIUSO, stadio 2 GATE NEGATIVO (2026-07-23)

Giro per portare la doppia estrazione per zona **sugli eventi** (autori/attribuzione), non più solo su punteggi/parziali/data. Incorpora una scoperta: la truth del gold identifica gli autori **per calottina** (il numero in tabella), mentre lo schema V3.x identificava il giocatore **solo per nome** (grafia manoscritta) — il campo più costoso della catena, e la zona dove la calottina si legge è esattamente la **storia cronometrica**. Il giro è a stadi con un **gate** dopo lo stadio 2 (ritaglio); il gate ha dato **negativo** e ci si è fermati lì, come da regola. Tutto **bench-only, default off**: `OCR_PROMPT_VERSION`, provider e modello di produzione **intatti**; `v3_4` **byte-identico** (sha `4e9751eded9b…`); nessuna migration; **spesa reale $0** (stadi 1–2 non fanno chiamate).

**Stadio 1 — CHIUSO e in repo (`b11ff4b`). Identità primaria per calottina.** Variante di prompt **V3.5** derivata da `v3_4` per sostituzione mirata (stessa disciplina di V3.2→V3.4, ogni altra zona identica byte-per-byte): sul GOL, sull'esclusione (EXCLUSION_20) e sull'espulsione definitiva (EXCLUSION_DEF) il numero di **calottina (`cap`) è l'identificativo PRIMARIO**, obbligatorio; il `player_name` è **secondario** (null ammesso quando la calottina c'è, e un nome null NON è un fallimento); il principio "null invece di invenzione" **non è indebolito** — calottina illeggibile → `cap` null, mai indovinata. A valle: `cap` opzionale in `schema.validate` (retrocompatibile); nuovo warning `validate_coherence` "calottina evento non nel roster" (mai un blocker); **riconciliazione per `(team, cap)`** esatta sul roster in `converters.get_events_data`, col nome come **fallback** solo quando la calottina manca — così un gol con nome misletto ma calottina buona si aggancia comunque all'atleta giusto (era il collo di bottiglia dichiarato §10.37). Nessun campo su `MatchEvent`, nessuna migration: `cap` vive solo nella pipeline di estrazione e nel bench; i consumer del publishing leggono chiavi esplicite, quindi `cap` è inerte lì. 24 test mock (`tests_ocr_cap_identity`).

**Stadio 2 — GATE NEGATIVO, ci si è fermati (`5f3b1ac`). Le frazioni fisse non reggono sui 6 casi.** Finora la "zona" del secondo passaggio era solo restrizione d'attenzione via prompt (immagine **intera** al modello, nessun ritaglio reale). Introdotto un ritaglio **vero** (`ocr_zone_crop.crop_zone` / `zone_box_pixels`, frazioni fisse della pagina; unica zona `storia_cronometrica` = metà destra del foglio orientato; `crop_zone_from_file` preprocessa prima di ritagliare). Verifica **economica** (guardando i ritagli, non con una chiamata): generati i ritagli dei casi gold e ispezionati. **Esito:** su 4/5 casi risolvibili — **unime pk8** e **olympic pk11** (gli unici due con ground-truth eventi), più bellator pk16 e pol-delta pk7 — `ImagePreprocessor` normalizza a landscape e il ritaglio inquadra la storia cronometrica **pulita**. Ma **sc-salerno pk15** NON viene orientato (resta ruotato 90° dopo il preprocessing) e il ritaglio a frazione fissa prende la **zona sbagliata**; **triscelon** (2026-04-25) non ha `db_report_pk` risolvibile a DB. La causa è l'**orientamento non normalizzato** — esattamente "le foto sono inquadrate diversamente" — che è la **registrazione su template**, un progetto a sé fuori dallo scope del giro. Per la regola del giro (fixed fractions che non reggono → fermarsi allo stadio 2) **stadi 3 e 4 NON eseguiti**: niente prompt zona-eventi, niente `compare_event_passes`, niente misura, **zero spesa**. I ritagli generati sono salvati fuori dallo scratchpad volatile in `media/ocr_zone_crops_8.24/` (gitignorato: contengono PII — nomi giocatori/ufficiali — quindi **non** committati, ma stabili su disco).

**Decisione tecnica registrata.** Il ritaglio opera sull'immagine **già preprocessata** (la stessa inviata al modello), non sul grezzo: sul grezzo l'orientamento varia caso per caso (portrait/landscape, ruotato) e nessuna frazione fissa reggerebbe. È il preprocessing a normalizzare 4/5; il residuo (sc-salerno) è un limite dell'auto-rotate di `ImagePreprocessor`, non del ritaglio.
